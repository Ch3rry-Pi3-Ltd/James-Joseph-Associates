...
The important question is:
    ...
    "Does the backend expose a real JobAdder OAuth callback URI that behaves
    clearly during setup?"

That matters because the callback URI must be registered exactly in the
JobAdder developer portal, and it needs to point at a live backend route rather
than an invented placeholder path.
...
- prove the route reports whether OAuth settings are ready for the next step
"""
...
def test_jobadder_callback_accepts_authorization_code_when_oauth_settings_are_not_ready(
    ...
    assert (
        "JOBADDER_CLIENT_ID" in payload["next_step"]
    )
...
def test_jobadder_callback_reports_oauth_configuration_ready_when_required_settings_exist(
    ...
    assert (
        "token exchange and token storage" in payload["next_step"]
    )
