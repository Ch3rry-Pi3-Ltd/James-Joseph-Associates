...
1. Persist one Dropbox `.eml` provenance record and link it where identity is
   clean enough.
2. Decide the next Outlook step after the first fully linked `tw394`
   persistence proof:
...
3. Chunk/embed the persisted `tw398` job-spec document.
4. Formalise duplicate/latest-file policy in the canonical data model and
   ingestion rules.
5. Keep Microsoft Graph as the primary mailbox source; use Dropbox as archive
